...
var LoHangViewModel = function () {
    ...
    self.SelectedCol = ko.observableArray([]);
    ...
    //    /*self.ListHeader(self.ListHeaderInit);*/
    //}
    self.GetSelectedCol = function () {
        if (localStorage.getItem('DMLoHangHeader') === null) {
            self.SelectedCol = ko.observableArray(['colMaHang', 'colTenHang', 'colDonViTinh', 'colNhomHang',
                'colGiaBan', 'colSoLuongTon', 'colGiaVon', 'colGiaTriTon', 'colMaLoHang', 'colNgaySanXuat',
                'colNgayHetHan', 'colTrangThai', 'colNgayConHan']);
            localStorage.setItem('DMLoHangHeader', JSON.stringify(self.SelectedCol()));
        }
        else {
            self.SelectedCol = ko.observableArray(JSON.parse(localStorage.getItem('DMLoHangHeader')));
        }
    };
    self.GetSelectedCol();
    self.SelectedCol.subscribe(function (newVal) {
        self.ListHeaderInit().filter(p => self.SelectedCol().contains(p.colName)).map(p => p.colShow = true);
        self.ListHeaderInit().filter(p => !self.SelectedCol().contains(p.colName)).map(p => p.colShow = false);
        localStorage.setItem('DMLoHangHeader', JSON.stringify(self.SelectedCol()));
    }, null, "arrayChange");
}
