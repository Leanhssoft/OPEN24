...
var workTable = new Vue({
    ...
    methods: {
            ...
            ajaxHelper(self.HoaDonAPI + 'GetCTHoaDon_afterDatHang?idHoaDon=' + hdDB.ID).done(function (data) {
                if (data.length > 0) {
                    ...
                    for (let i = 0; i < cthd.length; i++) {
                        ...
                        ctNew = self.AssignNVThucHien_toCTHD(ctNew);
                        ctNew.ThanhPhan_DinhLuong = self.AssignTPDinhLuong_toCTHD(ctNew);
        ...
        CheckHoaDon_DaXuLy: async function (idHoaDon, loaiCheck) {
            let self = this;
            let xx = await $.getJSON(self.GaraAPI + 'CheckHoaDon_DaXuLy?idHoaDon=' + idHoaDon + '&loaiHoaDon=' + loaiCheck).done(function () {
            }).then(function (x) {
                return x;
            })
            return xx;
        ...
        CapNhatHoaDon: async function (item, loaiHD) {
            ...
            if (loaiHD === 25) {
                check = await self.CheckHoaDon_DaXuLy(item.ID, 6);
                if (check) {
            ...
            // get hoahong hoadon of nhanvien
            await vmHoaHongHoaDon.GetChietKhauHoaDon_byID(item, null);
            ...
            var cthd = cthdDB.sort(function (a, b) {
                var x = a.SoThuTu,
                    y = b.SoThuTu;
                return x < y ? -1 : x > y ? 1 : 0;
            });
        ...
        GetChiTietHD_fromDB: async function (idHoaDon) {
            if (!commonStatisJs.CheckNull(idHoaDon)) {
                let self = this;
                const xx = await ajaxHelper(self.HoaDonAPI + 'SP_GetChiTietHD_byIDHoaDon_ChietKhauNV?idHoaDon=' + idHoaDon, 'GET').done()
                    .then(function (data) {
        ...
        PrintMerger: function (loaiHD = 25) {
            ...
            ajaxHelper('/api/DanhMuc/BH_HoaDonAPI/' + 'GetChiTietHD_MultipleHoaDon?arrID_HoaDon=' + self.arrID_HDPrint, 'GET').done(function (data) {
                if (data !== null) {
                    ...
                    if (self.listData.CongTy.length > 0) {
                        objHD.LogoCuaHang = Open24FileManager.hostUrl + self.listData.CongTy[0].DiaChiNganHang;
                        objHD.TenCuaHang = self.listData.CongTy[0].TenCongTy;
                        objHD.DiaChiCuaHang = self.listData.CongTy[0].DiaChi;
                        objHD.DienThoaiCuaHang = self.listData.CongTy[0].SoDienThoai;
                    }

                    let cn = self.GetInforChiNhanh();
                    objHD.DiaChiChiNhanh = cn.DiaChiChiNhanh;
                    objHD.DienThoaiChiNhanh = cn.DienThoaiChiNhanh;
