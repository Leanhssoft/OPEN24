...
namespace banhang24.Areas.DanhMuc.Controllers
{
    public class BaseApiController : ApiController
    {
        ...
        public bool CheckRoleIsAdmin()
        {
            ...
            {
                HT_NguoiDung nguoidung = new System.Web.Script.Serialization.JavaScriptSerializer()
                                            .Deserialize<HT_NguoiDung>(account);
                isAdmin = nguoidung.LaAdmin;
            }
        ...
        public bool CheckUserHasAccessToGroup(string idnhomhang, Guid iddonvi)
        {
            using (SsoftvnContext db = SystemDBContext.GetDBContext())
            {
                var nv_nd = banhang24.Hellper.contant.GetUserCookies();
                ...
                // Tách danh sách ID nhóm hàng từ chuỗi
                var idNhomHangList = idnhomhang.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries)
                                                 .Select(id => id.Trim())
                                                 .ToList();

                // Lấy danh sách tên nhóm hàng hóa không dấu
                var groupNames = db.DM_NhomHangHoa
                    .Where(x => idNhomHangList.Contains(x.ID.ToString()))
                    .Select(x => new { x.ID, x.TenNhomHangHoa_KhongDau })
                    .ToList();

                // Kiểm tra quyền cho từng nhóm hàng
                bool hasAccessToGroup = false;

                // Lấy danh sách các nhóm người dùng của người dùng hiện tại
                var userGroups = db.HT_NguoiDung_Nhom
                    .Where(x => x.ID_DonVi == iddonvi && x.IDNguoiDung == idUser)
                    .Select(x => x.IDNhomNguoiDung)
                    .ToList();

                // Lấy danh sách quyền của các nhóm người dùng
                var groupPermissions = db.HT_Quyen_Nhom
                    .Where(x => userGroups.Contains(x.ID_NhomNguoiDung))
                    .Select(x => x.MaQuyen)
                    .ToList();

                // Kiểm tra quyền xem nhóm hàng cho từng nhóm
                foreach (var group in groupNames)
                {
                    if (!string.IsNullOrEmpty(group.TenNhomHangHoa_KhongDau))
                    {
                        // Tạo mã quyền theo cú pháp
                        string permissionCode = "NhomHangHoa_XemNhom_" + group.TenNhomHangHoa_KhongDau;

                        // Kiểm tra quyền xem nhóm hàng
                        if (groupPermissions.Contains(permissionCode)) /*groupPermissions.Contains("NhomHangHoa_QuyenXemNhom") &&*/
                        {
        ...
        public IHttpActionResult GetListColumnInvoices(int loaiHD)
        {
            ...
            switch (loaiHD)
            {
                ...
                case 13:
                    ...
                    break;
                case 33:
                    data = commonEnum.ListColumnNhapThuHoiPT.ToList();
                    break;
                case 6:
