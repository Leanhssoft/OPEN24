...
namespace banhang24.Controllers
{
    ...
    public class ThietLapController : BaseController
    {
        ...
        public ActionResult _suanguoidung(string id)
        {
            return PartialView();
        }
        ...
        public ActionResult EventChangeLoaiMauIn(string id, string MaChungTu)
        {
            using (SsoftvnContext db = SystemDBContext.GetDBContext())
            {
                ...
                {
                    var selectedKhoGiay = commonEnum.DanhSachTypeMauIn.Any(o => o.Value.Equals(result.KhoGiay)) ? commonEnum.DanhSachTypeMauIn.FirstOrDefault(o => o.Value.Equals(result.KhoGiay)).Key : (int?)null;
                    var dsMauIn = db.DM_MauIn.Where(o => o.ID_LoaiChungTu == result.ID_LoaiChungTu && o.ID_DonVi == objUser_Cookies.ID_DonVi).Select(o => new ListTypeMauIn { Key = o.ID, Value = o.TenMauIn }).ToList();
            ...
            using (SsoftvnContext db = SystemDBContext.GetDBContext())
            {
                ...
                try
                {
                    ...
                    if (checkExits)
                    {
                        ...
                        var chungTu = db.DM_LoaiChungTu.FirstOrDefault(o => o.MaLoaiChungTu.Equals(model.MaChungTu));
                        int chungTuID = chungTu != null ? chungTu.ID : 0;
                        var MauInId = InsertMauIn(model.Name, commonEnum.DanhSachTypeMauIn.FirstOrDefault(o => o.Key.Equals(selectedKhoGiay)).Value, chungTuID, content, db, objUser_Cookies);
        ...
        public ActionResult InMaVach(Guid mauId, string masp, string tensp, string gia, int? item, Guid hanghoaId, Guid? banggiaId, int sobanghi = 3)
        {
            ...
            return View(model);
            //}
            //else { return ResultGetFalse(string.Empty, "Không lấy được file in mã vạch"); }
        }
        public ActionResult InTemMaVach(Guid mauId, Guid ID_SanPham)
        {
            //SsoftvnContext db = SystemDBContext.GetDBContext();
            //var fileMau = db.DM_MauIn.FirstOrDefault(o => o.ID == mauId);
            //var sp = db.DM_HangHoa.FirstOrDefault(o => o.ID == ID_SanPham);
            //if (sp == null)
            //    return null;
            //var rowtable = fileMau.DuLieuMauIn;
            //var tencongty = db.HT_CongTy.First().TenCongTy;
            //var gia= sp.DonViQuiDois.Any()?sp.DonViQuiDois.FirstOrDefault(p=>p.LaDonViChuan == true).GiaBan *  : 
            //var model = new InMaVach()
            //{
            //    GiaSP = gia,
            //    MaSP = masp,
            ...
            //if (rowtable.Contains("{MaHangHoaN}") || rowtable.Contains("{MaHangHoa}"))
            //{
            //    rowtable = rowtable.Replace("{MaHangHoa}", "<div>" + masp + "</div>");
            //}
            //if (rowtable.Contains("{GiaN}") || rowtable.Contains("{Gia}"))
            //{
            //    rowtable = rowtable.Replace("{Gia}", "<div >" + gia + "</div>");
            //}
            //if (rowtable.Contains("{TenSanPhamN}") || rowtable.Contains("{TenSanPham}"))
            //{
            //    rowtable = rowtable.Replace("{TenSanPham}", "<div >" + tensp + "</div>");
            //}
            //if (rowtable.Contains("{TenCuaHangN}") || rowtable.Contains("{TenCuaHang}"))
            //{

            //    rowtable = rowtable.Replace("{TenCuaHang}", "<div >" + tencongty + "</div>");
            //}
            //model.Link = "/Barcode.ashx?ht=30&vCode=" + masp;
            //rowtable = rowtable.Replace("{MaVach}", "<div style='text-align: center'><img style='max-width:132px' src=\" " + model.Link + "\"  height='24' /></div>");
            //model.Table = rowtable;
            return View();
            //}
